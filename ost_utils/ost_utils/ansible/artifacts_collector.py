...
def collect(domain_name, artifacts_list, target_dir):
    artifacts_list_string = ','.join(artifacts_list)
    module_mapper = mm.module_mapper_for(domain_name)
    archive_name = "artifacts.tar.gz"
    local_archive_dir = os.path.join(target_dir, "test_logs", domain_name)
